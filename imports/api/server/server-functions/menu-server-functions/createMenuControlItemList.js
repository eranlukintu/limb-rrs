...
export const createMenuControlItemList = function() {
	...
	topLevelMenuControlItemRows.forEach(function(t) {
		const foundMenuControlVariableRow = MENUCONTROLVARIABLESROWS.findOne({sourceDrowId: t.dRowId});
		if(!foundMenuControlVariableRow){
			...
			console.log("Menu control item row already exists");
		}		
	});
